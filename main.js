...
function createWindow() {
    ...
    if (app.isPackaged) {
        // Production: Load the index.html from the build directory
        win.loadFile(path.join(__dirname, 'dist/index.html'));
    } else {
        // Development: Load from the Vite server
...
function clearSchedule(sessionId, webContents) {
    ...
    if (schedule) {
        ...
        if (webContents) {
            ...
            webContents.send('backend-log', { type: 'info', message: msg, timestamp: Date.now(), sessionId })
        };
    }
}
